...
class PublicAPIGenerator:
    ...
    def _is_pixeltable_type(self, cls):
        ...
        return cls.__name__ in type_names or "type_system" in module_name or "types" in module_name.lower()
                    ...
                    for name, obj in inspect.getmembers(cls):
                        if not name.startswith("_") or name in ["__init__", "__call__"]:
                            if inspect.ismethod(obj) or inspect.isfunction(obj):
                                if name not in ["__init__"]:  # Skip constructor
    ...
    def generate(self, pages) -> Path:
        ...
        for category in self.api_entries:
            self.api_entries[category] = sorted(list(set(self.api_entries[category])))
